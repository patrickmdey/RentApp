...
function ReviewForm(props: { article: Article; review?: Review }) {
  ...
  const [rating, setRating] = useState(
    review === undefined ? 1 : review.rating
  );
  ...
  const {
    ...
    isSuccess,
    isLoading,
  } = useListImages(article.imagesUrl);
  ...
  if (
    review != null &&
    loggedUser != null &&
    review.renterUrl != loggedUser.url
  ) {
          ...
          <Col md={7} lg={7}>
            ...
            <Form onSubmit={handleSubmit(onSubmit)}>
              ...
              <Row>
                <Button
                  type="submit"
                  className="rounded btn-block bg-color-action btn-dark mt-3"
                >
