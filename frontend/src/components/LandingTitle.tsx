...
export default function LandingTitle() {
    ...
    function RenderLoggedUser() {
        return (
            <Col md="6" className="justify-content-center align-items-center">
                <h1>
                    {
                        strings.formatString(strings.collection.landing.titleLoggedIn,firstName)
                    }
                </h1>
                {
                    requests.pending == 0 ?
                        <p className="lead">
                            {strings.collection.landing.textNoRequests}
                        </p>
                        :
                        <div>
                            <p className="lead">
                                { strings.formatString(strings.collection.landing.pendingRequests,requests.pending.toString())}
                            </p>
                            <p className="lead">
                                { strings.formatString(strings.collection.landing.acceptedRequests,requests.accepted.toString())}
                            </p>
                            <p className="lead">
                                { strings.formatString(strings.collection.landing.rejectedRequests,requests.rejected.toString())}
                            </p>
                        </div>
    ...
    return (
        <Row className=" landing-title-container">
            {isAuthenticated ? RenderLoggedUser() : RenderNotLoggedUser()}
