...
function FilterCard(props: FilterCardProps) {
  ...
  const { data: locations, isSuccess: locationsIsSucc } =
    useListUsedLocations();
        ...
        orderOptions && (
          <Card className="card-style filters-card col-md-3 col-lg-3 col-12">
            <Card.Title className="w-100 d-flex align-items-center justify-content-between">
              <h4 className="color-rentapp-black m-0">
            ...
            <Form onSubmit={handleSubmit(onSubmit)}>
              <Card.Body style={{ padding: "0px" }}>
                ...
                <FormSelect
                  ...
                  options={
                    categories
                      ? [UNSELECTED_DEFAULT, ...categories].map(
                          ({ id, description }) => [id.toString(), description]
                        )
                      : []
                  }
                />
                <FormSelect
                  ...
                  options={
                    orderOptions
                      ? orderOptions.map(({ id, description }) => [
                          id,
                          description,
                        ])
                      : []
                ...
                <FormSelect
                  ...
                  options={
                    locations
                      ? [UNSELECTED_DEFAULT, ...locations].map(
                          ({ id, name }) => [id, name]
                        )
                      : []
                  }
